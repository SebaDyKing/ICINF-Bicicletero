...
export const guardBodyValition = Joi.object({
  rut : Joi.string().required().min(12).max(12).pattern(/^(\d{1,2}\.\d{3}\.\d{3}-[\dkK]|\d{7,8}-[\dkK])$/).required()    .custom((value, helpers) => {
      if (!validarRut(value)) {
  ...
  nombre: Joi.string().min(3).pattern(/^[A-Za-zÁÉÍÓÚáéíóúÑñ\s]+$/).required().messages({
    'string.base': `El campo nombre debe ser un texto.`,
    'string.min': `El campo nombre debe tener al menos 3 caracteres.`,
    'any.required': `El campo nombre es obligatorio.`,
    'string.pattern.base': 'El campo nombre no puede contener números ni símbolos.',
    'string.empty': `El campo nombre no puede estar vacío.`,
  }),

  apellido: Joi.string().min(3).pattern(/^[A-Za-zÁÉÍÓÚáéíóúÑñ\s]+$/).required().messages({
    ...
    'string.pattern.base': 'El campo apellido no puede contener números ni símbolos.',
    'string.min': `El campo apellido debe tener al menos 3 caracteres.`
  }),

  telefono: Joi.string().min(15).max(15).required().pattern(/^\+56 9 \d{4} \d{4}$/).messages({
    ...
    'string.pattern.base': 'El teléfono debe tener el formato +56 9 XXXX XXXX y solo contener números.',
